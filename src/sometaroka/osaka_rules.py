...
def translate_text_osaka(input_text):
    ...
    while i < len(words):
        ...
        if word == "よ":
            ...
            if words[i - 1] == "た":
                ...
                if words[i - 2] == "て":
                    ...
                    word = "で"
            elif is_pronoun(words[i - 1]) and (
                i + 1 < len(words) and (words[i - 1] != "なん" or words[i - 1] != "何")
            ):
                # 直前が代名詞のとき「よ」を「や」に変換
                word = "や"
        ...
        if word == "ない":
            if words[i - 1] == "し":
                ...
                if (
                    i + 1 < len(words)
                    and is_verb_before_nai(words[i - 1])
                    and words[i + 1] == "ん"
                ):
                    word = "へん"
                    i += 1
        ...
        if word == "だ":
            if i < len(words) - 1 and words[i + 1] == "よ":
                ...
                if i + 2 < len(words) and (words[i + 2] == "ね" or words[i + 2] == "な"):
                    if (
                        is_noun(words[i - 1])
                        or is_Adverb(words[i - 1])
                        or is_postposition(words[i - 1])
                        or is_adjective(words[i - 1])
                    ):
                        if words[i - 2] == "ない":
                            word = "ねんな"
                            i += 2
            ...
            # 「だ」の処理
                ...
                # 「だね：だね」の処理；なぜか「あれ」が変換できない
                    ...
                    processed_words.pop()
                    processed_words.pop()
                elif words[i - 1] == "ん" and (
                    is_verb_before_nai(words[i - 2]) or is_adjective(words[i - 2])
                ):
                    processed_words.pop()
                    word = "ねん"
                    ...
                    words[i - 2] == "な"
                    or is_noun(words[i - 1])
                    or is_Adverb(words[i - 1])
                    or is_postposition(words[i - 1])
                    or is_adjective(words[i - 1])
                    or is_pronoun(words[i - 1])
                ):
        ...
        if word == "じゃん":
            if (
                words[i - 1] == "ない"
                or is_verb_before_nai(words[i - 1])
                or is_verb_before_nai(words[i - 2])
                or is_noun(words[i - 1])
                or is_Adverb(words[i - 1])
                or is_postposition(words[i - 1])
                or is_adjective(words[i - 1])
        ...
        if word == "た":
            if i + 1 < len(words) and words[i + 1] == "の":
                if i + 2 < len(words) and (words[i + 2] == "?" or words[i + 2] == "？"):
                    ...
                    processed_words.pop()
                    processed_words.pop()
                elif words[i - 1] == "経っ" and (
                    i + 2 < len(words) and words[i + 2] == "か"
                ):
                    word = "経つん"
                    processed_words.pop()
                    i += 1
        ...
        if word == "つまずい" and (
            ...
            if i + 1 < len(words) and (words[i + 1] == "た" or words[i + 1] == "て"):
                word = "ぐねっ"
        if (word == "Y" or word == "y") and (
            i + 1 < len(words) and (words[i + 1] == "シャツ" or words[i + 1] == "しゃつ")
        ):
            word = "カッターシャツ"
            i += 1
        if word == "刺さ":
    ...
    sheet = client.open_by_key("166R1W9WwkelPu861pCQkjIfsmWF7xuIrqUy4BpB_ixY").sheet1
